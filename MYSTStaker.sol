...
contract MYSTStaker is IStaking, Ownable {
    ...
    constructor(IERC20 stakingToken, IERC20 distributionToken, uint256 maxUnlockSchedules,
        ...
        require(startBonus_ <= 10**BONUS_DECIMALS, 'MYSTStaker: start bonus too high');
        // If no period is desired, instead set startBonus = 100%
        // and bonusPeriod to a small value like 1sec.
        require(bonusPeriodSec_ != 0, 'MYSTStaker: bonus period is zero');
        require(initialSharesPerToken > 0, 'MYSTStaker: initialSharesPerToken is zero');
    ...
    function _stakeFor(address staker, address beneficiary, uint256 amount) private {
        require(amount > 0, 'MYSTStaker: stake amount is zero');
        require(_stakeEnable, 'Stake not enabled yet');
        require(beneficiary != address(0), 'MYSTStaker: beneficiary is zero address');
        require(totalStakingShares == 0 || totalStaked() > 0,
                'MYSTStaker: Invalid state. Staking shares exist, but no staking tokens do');

        uint256 mintedStakingShares = (totalStakingShares > 0)
            ? totalStakingShares.mul(amount).div(totalStaked())
            : amount.mul(_initialSharesPerToken);
        require(mintedStakingShares > 0, 'MYSTStaker: Stake amount is too small');
        ...
        require(_stakingPool.token().transferFrom(staker, address(_stakingPool), amount),
            'MYSTStaker: transfer into staking pool failed');
    ...
    function _unstake(uint256 amount) private returns (uint256) {
        ...
        // checks
        require(amount > 0, 'MYSTStaker: unstake amount is zero');
        require(totalStakedFor(msg.sender) >= amount,
            'MYSTStaker: unstake amount is greater than total user stakes');
        uint256 stakingSharesToBurn = totalStakingShares.mul(amount).div(totalStaked());
        require(stakingSharesToBurn > 0, 'MYSTStaker: Unable to unstake amount this small');
        ...
        require(_stakingPool.transfer(msg.sender, amount),
            'MYSTStaker: transfer out of staking pool failed');
        require(_unlockedPool.transfer(msg.sender, rewardAmount),
            'MYSTStaker: transfer out of unlocked pool failed');

        emit Unstaked(msg.sender, amount, totalStakedFor(msg.sender), "");
        emit TokensClaimed(msg.sender, rewardAmount);

        require(totalStakingShares == 0 || totalStaked() > 0,
                "MYSTStaker: Error unstaking. Staking shares exist, but no staking tokens do");
        return rewardAmount;
    }
    ...
    function lockTokens(uint256 amount, uint256 durationSec) external onlyOwner {
        require(unlockSchedules.length < _maxUnlockSchedules,
            'MYSTStaker: reached maximum unlock schedules');
        ...
        require(_lockedPool.token().transferFrom(msg.sender, address(_lockedPool), amount),
            'MYSTStaker: transfer into locked pool failed');
    ...
    function unlockTokens() public returns (uint256) {
        ...
        if (unlockedTokens > 0) {
            require(_lockedPool.transfer(address(_unlockedPool), unlockedTokens),
                'MYSTStaker: transfer out of locked pool failed');
